...
class DDPG(object):
    ...
    def __init__(self, sess, state_dim, action_dim, max_action, min_action, actor_learning_rate, critic_learning_rate, tau, RANDOM_SEED, device = '/cpu:0'):
        ...
        with tf.variable_scope('learning_rate'): 
            ...
            self.actor_decay_learning_rate = tf.train.exponential_decay(self.actor_learning_rate, self.global_step, 200000, 0.96, staircase=True)
            self.critic_decay_learning_rate = tf.train.exponential_decay(self.critic_learning_rate, self.global_step, 200000, 0.96, staircase=True)
    ...
    def _build_net(self,scope):
        ...
        with tf.device(self.device):        
            ...
            with tf.variable_scope(scope + '/actor'):
                ...
                # scaled_a = tf.clip_by_value(a,self.min_action,self.max_action)#
                       
        saver = tf.train.Saver()
        return v, a, scaled_a, saver

    def train(self, s_batch, a_batch, r_batch, t_batch, s2_batch, MINIBATCH_SIZE):
        # This is the typical training of the DDPG
    ...
    def train_inverted_gradient(self, s_batch, a_batch, r_batch, t_batch, s2_batch, MINIBATCH_SIZE):
        # if you use this method for training, instead of training the DDPG as usual, you will 
        # train it by using inverted gradients, refer to the paper: 
...
if __name__ == '__main__':
    ...
    # import gym_foo
    # ENV_NAME = 'nessie_end_to_end-v0' # I have a code of nessie stripped down of all ROS stuff
    max_action = 2.
    min_action = -2.
    epochs = 500
    epsilon = 1.0
    ...
    with tf.Session() as sess:
        ...
        agent = DDPG(sess, state_dim, action_dim, max_action, min_action, ACTOR_LEARNING_RATE, CRITIC_LEARNING_RATE, TAU, RANDOM_SEED,device=DEVICE)
        sess.run(tf.global_variables_initializer())
        # check that we are effectively updating the parameters
        #print(agent.a_params_target[0].eval()[0][0],agent.a_params[0].eval()[0][0])
        ...
        for i in range(epochs):
            ...
            while (not done):
                step += 1
                action = agent.predict_action(np.reshape(state,(1,state_dim)))
                action1 = action
                ...
                if replay_buffer.size() > MINIBATCH_SIZE + 30000:
                    s_batch, a_batch, r_batch, t_batch, s2_batch = replay_buffer.sample_batch(MINIBATCH_SIZE)
                    # Typical DDPG training
                    agent.train(s_batch, a_batch, r_batch, t_batch, s2_batch,MINIBATCH_SIZE)
                    # Training with inversion of gradients
                    # agent.train_invert_gradient(s_batch, a_batch, r_batch, t_batch, s2_batch,MINIBATCH_SIZE)
                else:
                    epsilon = 1.
